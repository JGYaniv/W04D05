...
def brute_sum?(arr, target)
    arr.each_with_index do |el1, idx1|
        (idx1+1...arr.length).each do |idx2|
            if arr[idx2] + el1 == target
                return true
            end
        end
    end
...
arr = [0, 1, 5, 7]
# p brute_sum?(arr, 6) # => should be true
# p brute_sum?(arr, 10) # => should be false
...
arr = [0, 1, 5, 7]
# sort_sum?(arr, 6) # => should be true
# sort_sum?(arr, 10) # => should be false
